function varargout = simpleGU(varargin)
% SIMPLEGU MATLAB code for simpleGU.fig
%      SIMPLEGU, by itself, creates a new SIMPLEGU or raises the existing
%      singleton*.
%
%      H = SIMPLEGU returns the handle to a new SIMPLEGU or the handle to
%      the existing singleton*.
%
%      SIMPLEGU('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SIMPLEGU.M with the given input arguments.
%
%      SIMPLEGU('Property','Value',...) creates a new SIMPLEGU or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before simpleGU_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to simpleGU_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help simpleGU

% Last Modified by GUIDE v2.5 08-Jul-2021 23:17:39

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @simpleGU_OpeningFcn, ...
                   'gui_OutputFcn',  @simpleGU_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before simpleGU is made visible.
function simpleGU_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to simpleGU (see VARARGIN)

% Choose default command line output for simpleGU
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes simpleGU wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = simpleGU_OutputFcn(hObject, eventdata, handles) 
